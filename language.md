...
- `CREATE template=<TemplateId[,SvgTemplateId]> id=<TokenId> initials=<XX> name=<Label> faction=<pc|ally|npc|enemy|team> bg=#RRGGBB fg=#RRGGBB speed=<ft> type=<structure|creature|object> size=<cells> level=<n> hp=<current> hpMax=<max> info="<free text>" @ A1,B2,...` — spawn tokens from templates (first template id drives the 3D model; optional second drives the SVG cap). `id` expands per placement; `initials` defaults to the first letters of the id; `name` renders on the token’s side band; `faction` (aliases: side/team) tints the cylinder green for pc/ally, red for npc/enemy; `bg`/`fg` override template colors; `speed` sets move speed (defaults to 12); `type` is a free-form category (e.g., `structure`); `size` is the footprint width on the smallest dimension in cells. If `info` is omitted, but `level`/`hp`/`hpMax` are provided, the band shows e.g. `Lvl 5 HP 12/18`. Tokens start with `hp` (or `hpMax` if `hp` is omitted); `hpMax` is treated as max HP.
